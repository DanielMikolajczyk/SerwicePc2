...
@section('content')
  <div class="w-full mx-auto">
    <div class="my-5">
      Logo
      <a href="{{ route('clienttype.create') }}" class="ml-3">
        Create
      </a>
    </div>
            ...
            <tbody class="text-gray-600 text-sm font-light">
              @forelse($clientTypes as $clientType)
                <tr class="border-b border-gray-200 hover:bg-gray-100 font-sm text-center">
                  <td class="py-3 px-4">
                    <span class="font-medium">{{ $clientType->name }}</span>
                  </td>
                  <td class="py-3 px-4">
                    <div class="flex item-center justify-center">
                      ...
                      <div class="w-4 mr-2 transform hover:text-purple-500 hover:scale-110">
                        <form method="post"
                          action="{{ route('clienttype.destroy', $clientType->id) }}" 
                          id="delete_form_{{ $clientType->id }}">
                          @csrf
                          @method('DELETE')
                          <a href="javascript:{}"
                            onclick="document.getElementById('delete_form_{{ $clientType->id }}').submit();">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
